...
Proof.
  ...
  - destruct m.
    + reflexivity.
    + rewrite add_comm in H. discriminate H. Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros. unfold not. intros H1. apply H in H1. 
  unfold not in H0. apply H0 in H1.
  assumption. Qed. 
(** [] *)
...
Proof.
  intros.
  split.
  - rewrite H; rewrite H0. intros; assumption.
  - rewrite H; rewrite H0. intros; assumption. Qed.
(** [] *)
...
Proof.
  intros.
  unfold not.
  intros [x Hx]. apply Hx in H. apply H. Qed.
(** [] *)
...
Proof.
  ...
  - intros. destruct H. rewrite H. apply H0. Qed. 
...
Proof.
  ...
  - intros. unfold not. intros.
    rewrite H0 in H. Search eqb. rewrite eqb_refl in H. discriminate H.
  - intros. rewrite <- not_true_iff_false. unfold not. unfold not in H.
  intros. rewrite eqb_eq in H0. apply H in H0. apply H0. Qed.
(** [] *)
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool)
  ...
  | x :: t0, [] => false
  | [], y :: t1 => false
  | x :: t0, y :: t1 => eqb x y && eqb_list eqb t0 t1 
  end.

Lemma list_eq_comp :
  forall X : Type,  
  forall x0 x1 : X, 
  forall l0 l1 : list X, x0 :: l0 = x1 :: l1 -> x0 = x1 /\ l0 = l1.
Proof.
  intros.
...
Proof.
  ...
  - intros. generalize dependent l2. induction l1.
    ...
    + intros. destruct l2.
      ...
      apply IHl1 in H1; apply H in H0. rewrite H1. rewrite H0. reflexivity. }
  - intros.  generalize dependent l2. induction l1.
    + intros. destruct l2.
      { reflexivity. }
      { discriminate H0. }
    + intros. destruct l2.
      { discriminate H0.  }
      { simpl. injection H0 as H1 H2. rewrite andb_true_iff.
      split.
      - apply H in H1. apply H1.
      - apply IHl1 in H2. apply H2. } Qed.  
...
Proof.
  ...
  - intros. induction l.
    + reflexivity.
    + simpl in H. destruct H. simpl. rewrite andb_true_iff. split.
      { apply H.  }
      { apply IHl in H0. apply H0. } Qed.
...
Lemma demorgans: forall P Q : Prop, 
