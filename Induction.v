...
(** **** Exercise: 2 stars, standard, especially useful (add_comm_informal)

    Translate your solution for [add_comm] into an informal proof:

    Theorem: Addition is commutative.

    (for all Natural numbers [n],[m], [n + m = n + m])

    Proof: by induction on [n] 

    -First show the proposition holds for [n = 0]:
    
    0 + m = m + 0

    This follows directly from the definition of [+]

    - Then show, assuming [n' + m = m + n'] where [n = S n'], that 
      [S n' + m = m + S n']

      we know by the above proof plus_n_Sm that [forall n,m: n + S m = S(n + m)]
      thus by substituting the right side of this equation with our
      proposition we can find:

      [S n' + m = S (m + n')] 

      and by substituting the left side of our inductive hypothesis we find:

      [S n' + m = S (n' + m)]
      
      This equation is true by the definition of [+]. Qed.
...
Proof.
  ...
  - simpl. rewrite <- mult_n_Sm. rewrite IHn. rewrite add_comm. reflexivity. Qed. 
(** [] *)
...
Proof.
  intros.
  simpl. rewrite add_0_r.  reflexivity. Qed.
...
Proof.
  ...
  - rewrite <- add_comm. rewrite <- plus_n_Sm. simpl. 
    ...
    rewrite H. rewrite IHn. rewrite add_assoc. reflexivity. Qed. 
...
Proof.
  ...
  - simpl. rewrite !add_0_r. rewrite IHb. rewrite add_assoc. simpl.
  rewrite <- add_comm. rewrite add_assoc. reflexivity. Qed.
