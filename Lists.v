...
Fixpoint alternate (l1 l2 : natlist) : natlist.
 Admitted.
  ...
  | nil => match l2 with
    | nil => nil
    | y :: t => y :: alternate nil t
  end
  | x :: t1 => match l2 with
    | nil => x :: alternate nil t1
...
Fixpoint remove_one (v : nat) (s : bag) : bag
  := match s with
  | nil => nil
  | x :: t => match (eqb x v) with
    | true => t
    | false => x :: remove_one v t 
  end
  end.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool
  := match s1 with
  | nil => true
  | x :: t => match (member x s2) with
    | true => true && subset t (remove_one x s2)
    | false => false
  end
  end.
...
Proof.
  ...
  assert (H1 : l1 = rev (rev l1)).
   ...
   - rewrite H1. rewrite H. reflexivity. Qed.
...
(* Infinite, in fact there is no finitely sized thing of type baz *)
